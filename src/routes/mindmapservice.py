...
################################################################################
        ...
        for row in n_list:
            if(row['tag'] != "GuiMenu"):
                if type(row) == str and len(row) == 0:
                    continue
                if "custname" not in row:
                    row["custname"] = "object"+str(row["_id"])
                row["parent"] = [pid]
                req.append(InsertOne(row))
        if req:
            dbsession.dataobjects.bulk_write(req)
            queryresult = list(dbsession.dataobjects.find(
                {"parent": pid}, {"custname": 1, "_id": 1, "parent": 1}))
        else:
            return pid
        return queryresult
        ...
        if(obj != []):
            for i in range(len(obj)):
                ...
                if so["apptype"] == "WEB":
                    ...
                    legend = ['id', 'name', 'tag', 'left', 'top',
                              'height', 'width', 'text', 'class']
                    for i in obn.split(';'):
                        ob.append(getScrapeData(i))
                    ob = ";".join(ob).split(';')
                    ob = ob[1:2] + ob[3:]
                    if len(ob) < 4:
                        custnameToAdd.append(dodata)
                    ...
                    try:
                        for i in range(len(legend)):
                            if (i >= 4 and i <= 7):
                                if ob[i].isnumeric():
                                    dodata[legend[i]] = int(ob[i])
                            else:
                                if (ob[i] != "null") and (legend[i] not in dodata):
                                    dodata[legend[i]] = ob[i]
                    except:
                        pass
                    gettag = {"btn": "button", "txtbox": "input", "lnk": "a", "radiobtn": "radiobutton", "select": "select",
                              "chkbox": "checkbox", "lst": "list", "tab": "tab", "tree": "tree", "dtp": "datepicker", "tbl": "table", "elmnt": "label"}
                    tag = so["custname"].split("_")[-1]
                    if tag in gettag:
                        dodata["tag"] = gettag[tag]
                    if "class" in dodata:
                        dodata["class"] = dodata["class"].split("[")[0]
                    dodata["url"] = so["url"] if 'url' in so else ""
                    dodata["cord"] = so["cord"] if "cord" in so else ""
                elif so["apptype"] == "SAP":
                    if so["tag"] == "GuiOkCodeField":
                        so["tag"] = "input"
                    if so["tag"] == "GuiSimpleContainer":
                        so["tag"] = "scontainer"
                    if so["tag"] == "GuiLabel":
                        so["tag"] = "label"
                    dodata = {
    ...
    def getProjectType():
        ...
        try:
            ...
            if not isemptyrequest(requestdata):
                ...
                getProjectType = list(dbconn.find({"_id": ObjectId(projectid)}, {
                                      "type": 1, "releases.name": 1, "releases.cycles.name": 1, "releases.cycles._id": 1, "domain": 1}))
                dbconn = dbsession["projecttypekeywords"]
                getProjectTypeName = list(dbconn.find(
                    {"_id": ObjectId(getProjectType[0]["type"])}, {"name": 1}))
                res = {'rows': getProjectType,
    ...
    def getProjectIDs():
        ...
        try:
            requestdata = json.loads(request.data)
            app.logger.debug("Inside getProjectIDs. Query: " +
                             str(requestdata["query"]))
            if not isemptyrequest(requestdata):
                if len(projecttype_names) == 0:
                    result = list(dbsession.projecttypekeywords.find(
                        {}, {"_id": 1, "name": 1}))
                    for p in result:
                ...
                projectIDResult = list(dbconn.find(
                    {"_id": ObjectId(userid)}, {"projects": 1}))
                if(len(projectIDResult) != 0):
                    ...
                    for pid in prjids:
                        dbconn = dbsession["projects"]
                        prjDetail = list(dbconn.find({"_id": ObjectId(pid)}, {
                                         "_id": 1, "name": 1, "type": 1, "domain": 1, "releases.name": 1, "releases.cycles.name": 1, "releases.cycles._id": 1}))
                        if(len(prjDetail) != 0):
                            prjDetails['projectId'].append(
                                str(prjDetail[0]['_id']))
                            prjDetails['projectName'].append(
                                prjDetail[0]['name'])
                            prjDetails['appType'].append(
                                str(prjDetail[0]['type']))
                            prjDetails['appTypeName'].append(
                                projecttype_names[str(prjDetail[0]['type'])])
                            prjDetails['releases'].append(
                                prjDetail[0]["releases"])
                            prjDetails['domains'].append(
                                prjDetail[0]["domain"])
                            for rel in prjDetail[0]["releases"]:
                                for cyc in rel['cycles']:
                                    prjDetails['cycles'][str(cyc['_id'])] = [str(
                                        cyc['_id']), rel['name'], cyc['name'], ]
    ...
    def updateScreenname_ICE():
        ...
        try:
            ...
            if(screenid == None):
                queryresult = dbsession.screens.insert_one({"name": requestdata['screenname'], "projectid": ObjectId(requestdata['projectid']), "versionnumber": requestdata['versionnumber'], "parent": [], "createdby": ObjectId(requestdata['createdby']), "createdon": modifiedon, "createdbyrole": ObjectId(requestdata['createdbyrole']), "modifiedby": ObjectId(
                    requestdata['modifiedby']), "modifiedon": modifiedon, "modifiedbyrole": ObjectId(requestdata['modifiedbyrole']), "deleted": requestdata['deleted'], "createdthrough": requestdata['createdthrough'], "screenshot": requestdata['screenshot'], "scrapedurl": requestdata['scrapedurl']}).inserted_id
                    ...
                    for i in range(len(data1)):
                        for j in range(len(data2)):
                            if(data2[j]['custname'] == data1[i]['custname']):
                                data2[j]['custname'] = ObjectId(
                                    data1[i]['_id'])
                    queryresult = dbsession.testcases.insert_one({"name": requestdata['testcasename'], "screenid": ObjectId(requestdata['screenid']), "versionnumber": requestdata['versionnumber'], "createdby": ObjectId(requestdata['createdby']), "createdon": modifiedon, "createdbyrole": ObjectId(
                        requestdata['createdbyrole']), "modifiedby": ObjectId(requestdata['modifiedby']), "modifiedon": modifiedon, "modifiedbyrole": ObjectId(requestdata['modifiedbyrole']), "deleted": requestdata['deleted'], "steps": data2, "parent": requestdata["parent"]})
                res = {'rows': 'Success'}
            else:
    ...
    def getTasksJSON():
        ...
        try:
            ...
            if not isemptyrequest(requestdata):
                userid = requestdata["userid"]
                tasks = list(dbsession.tasks.find(
    ...
    def updateAccessibilitySelection():
        ...
        try:
            ...
            if not isemptyrequest(requestdata):
                ...
                dbsession.tasks.update_one({"_id": ObjectId(taskid)}, {
                                           '$set': {"accessibilityparameters": requestdata["accessibilityParameters"]}})
                res = {'rows': "success"}
            else:
                app.logger.warn(
                    "Empty data received. updateAccessibilitySelection")
        except Exception as e:
    ...
    def getScenarios():
        ...
        try:
            ...
            if not isemptyrequest(requestdata):
                ...
                moduledetails = list(dbsession.mindmaps.find(
                    {"_id": ObjectId(moduleid)}, {"testscenarios": 1}))
                scenarioids = []
                for mod in moduledetails:
                    if "testscenarios" in mod:
                        for sce in mod["testscenarios"]:
                            scenarioids.append(ObjectId(sce["_id"]))
                scenarioslist = list(dbsession.testscenarios.find(
                    {"_id": {"$in": scenarioids}}, {"name": 1}))
                ...
                for moduledata in requestdata['testsuiteDetails']:
                    if moduledata["testsuiteId"] is None:
                        ...
                        if moduledata['state'] == "renamed":
                            updateModuleName(
                                moduledata['testsuiteName'], projectid, moduledata["testsuiteId"], createdby, createdbyrole)
                    ...
                    for scenariodata in moduledata['testscenarioDetails']:
                        ...
                        if scenariodata['testscenarioid'] is None:
                            currentscenarioid = saveTestScenario(
                                projectid, scenariodata['testscenarioName'], versionnumber, createdby, createdbyrole, currentmoduleid)
                        else:
                            if scenariodata['state'] == "renamed":
                                updateScenarioName(
                                    scenariodata['testscenarioName'], projectid, scenariodata['testscenarioid'], createdby, createdbyrole)
                            ...
                            if screendata["screenid"] is None:
                                if "newreuse" in screendata:
                                    currentscreenid = getScreenID(
                                        screendata["screenName"], projectid)
                                    updateparent(
                                        "screens", currentscreenid, currentscenarioid, "add")
                                else:
                                    currentscreenid = saveScreen(
                                        projectid, screendata["screenName"], versionnumber, createdby, createdbyrole, currentscenarioid)
                            else:
                                if screendata["state"] == "renamed":
                                    updateScreenName(
                                        screendata['screenName'], projectid, screendata['screenid'], createdby, createdbyrole)
                                currentscreenid = screendata["screenid"]
                                if "reuse" in screendata and screendata["reuse"]:
                                    updateScreenAndTestcase(
                                        currentscreenid, createdby, createdbyrole)
                                    updateparent(
                                        "screens", currentscreenid, currentscenarioid, "add")
                                ...
                                if testcasedata["testcaseid"] is None:
                                    if "newreuse" in testcasedata:
                                        currenttestcaseid = getTestcaseID(
                                            currentscreenid, testcasedata['testcaseName'])
                                        updateparent(
                                            "testcases", currenttestcaseid, currentscreenid, "add")
                                    else:
                                        currenttestcaseid = saveTestcase(
                                            currentscreenid, testcasedata['testcaseName'], versionnumber, createdby, createdbyrole)
                                else:
                                    if testcasedata['state'] == "renamed":
                                        updateTestcaseName(
                                            testcasedata['testcaseName'], projectid, testcasedata['testcaseid'], createdby, createdbyrole)
                ...
                if action == "modify":
                    ...
                    for i in tasks_update:
                        ...
                        if i['startdate'].find('-') > -1:
                            i["startdate"] = datetime.strptime(
                                i["startdate"], "%d-%m-%Y")
                        if i['enddate'].find('-') > -1:
                            i["enddate"] = datetime.strptime(
                                i["enddate"], "%d-%m-%Y")
                    ...
                    if len(tasks_remove) > 0:
                        ...
                        dbsession.tasks.delete_many(
                            {"_id": {"$in": tasks_remove}})
                    res = {"rows": "success"}
                elif action == "updatestatus":
                    status = requestdata['status']
                    dbsession.tasks.update({"_id": ObjectId(requestdata["id"])}, {
                                           "$set": {"status": status}})
                    res = {"rows": "success"}
                elif action == "updatetaskstatus":
                    task = dbsession.tasks.find_one(
                        {"_id": ObjectId(requestdata["id"])})
                    history = []
                    ...
                    if(len(task["history"]) == 0):
                        requestdata["history"]["userid"] = ObjectId(
                            requestdata["history"]["userid"])
                    ...
                    dbsession.tasks.update({"_id": ObjectId(requestdata["id"])}, {"$set": {
                                           "status": status, "history": history, "assignedto": assignedto, "owner": owner, "reviewer": reviewer}})
                    res = {"rows": "success"}
                elif action == "delete":
                    dbsession.tasks.delete(
                        {"_id": ObjectId(requestdata["id"]), "cycle": ObjectId(requestdata["cycleid"])})
                    res = {"rows": "success"}
            else:
        ...
        for moduledata in requestdata['testsuiteDetails']:
            ...
            for scenariodata in moduledata['testscenarioDetails']:
                ...
                if scenariodata['testscenarioName'] in scenarionames:
                    error = "A project cannot have similar scenario names: " + \
                        scenariodata['testscenarioName']
                    break
                else:
                ...
                if scenariodata['testscenarioid'] is None:
                    if checkScenarioNameExists(projectid, scenariodata['testscenarioName']):
                        error = "A project cannot have similar scenario names: change " + \
                            scenariodata['testscenarioName']+" name"
                        break
                else:
                ...
                for screendata in scenariodata['screenDetails']:
                    ...
                    if screendata["screenid"] is None:
                        ...
                        screendata["screenid"] = getScreenID(
                            screendata["screenName"], projectid)
                        ...
                        if screenname != screendata["screenName"]:
                            if checkScreenNameExists(screendata["screenName"], projectid):
                                error = "Cannot rename screen to an existing screen name: " + \
                    ...
                    if screendata["screenName"] not in screen_testcase:
                        ...
                        for testcasedata in screendata['testcaseDetails']:
                            if testcasedata["testcaseName"] not in screen_testcase[screendata["screenName"]]:
                                screen_testcase[screendata["screenName"]].add(
                    ...
                    if screendata["screenid"] is not None:
                        for testcasedata in screendata['testcaseDetails']:
                            if testcasedata['testcaseid'] is None:
                                testcasedata['testcaseid'] = getTestcaseID(
                                    screendata["screenid"], testcasedata["testcaseName"])
                                ...
                                if testcasename != testcasedata["testcaseName"]:
                                    ...
                                    if testcaseid is not None:
                                        updateparent(
                                            'testcases', testcasedata['testcaseid'], screendata["screenid"], 'delete')
    ...
    def checkModuleNameExists(name, projectid):
        res = list(dbsession.mindmaps.find(
            {"projectid": ObjectId(projectid), "name": name}, {"_id": 1}))
        if len(res) > 0:
            return True
        else:
            return False
    ...
    def checkScreenNameExists(name, projectid):
        res = list(dbsession.screens.find(
            {"projectid": ObjectId(projectid), "name": name}, {"_id": 1}))
        if len(res) > 0:
            return True
        else:
    ...
    def updateparent(type,nodeid,parentid,action):
        if action=="add":
            if type=="scenarios":
                ...
                dbsession.testscenarios.update_one({'_id': ObjectId(nodeid)}, {
                                                   '$set': {'parent': updateparentlist}})
            elif type == "screens":
                parentlist = list(dbsession.screens.find(
                    {"_id": ObjectId(nodeid)}, {"parent": 1}))
                updateparentlist = parentlist[0]['parent']
                updateparentlist.append(ObjectId(parentid))
                dbsession.screens.update_one({'_id': ObjectId(nodeid)}, {
                                             '$set': {'parent': updateparentlist}})
            elif type == "testcases":
                parentlist = list(dbsession.testcases.find(
                    {"_id": ObjectId(nodeid)}, {"parent": 1}))
                updateparentlist = parentlist[0]['parent']
                updateparentlist += 1
                dbsession.testcases.update_one({'_id': ObjectId(nodeid)}, {
                                               '$set': {'parent': updateparentlist}})
        elif action == "delete":
            if type == "scenarios":
                parentlist = list(dbsession.testscenarios.find(
                    {"_id": ObjectId(nodeid)}, {"parent": 1}))
    ...
    def updateTestcaseIDsInScenario(currentscenarioid, testcaseidsforscenario):
        dbsession.testscenarios.update_one({'_id': ObjectId(currentscenarioid)}, {
                                           '$set': {'testcaseids': testcaseidsforscenario}})
        return

    def updateTestScenariosInModule(currentmoduleid, idsforModule):
        dbsession.mindmaps.update_one({"_id": ObjectId(currentmoduleid)}, {
                                      '$set': {'testscenarios': idsforModule}})
        return
    
    # def updateProjectIdInModule(currentmoduleid, projectid):
    #     dbsession.mindmaps.update_one({"_id": ObjectId(currentmoduleid)}, {
    ...
    def updateScreenAndTestcase(screenid, createdby, createdbyrole):
        createdon = datetime.now()
        dbsession.screens.update_one({"_id": ObjectId(screenid)}, {'$set': {"createdby": ObjectId(createdby), "createdbyrole": ObjectId(
            createdbyrole), "createdon": createdon, "modifiedby": ObjectId(createdby), "modifiedbyrole": ObjectId(createdbyrole), "modifiedon": createdon}})
        dbsession.testcases.update_one({"screenid": ObjectId(screenid)}, {'$set': {"createdby": ObjectId(createdby), "createdbyrole": ObjectId(
            createdbyrole), "createdon": createdon, "modifiedby": ObjectId(createdby), "modifiedbyrole": ObjectId(createdbyrole), "modifiedon": createdon}})
        return
    ...
    def getScreens():
        ...
        try:
            ...
            if not isemptyrequest(requestdata):
                ...
                screendetails = list(dbsession.screens.find(
                    {"projectid": projectid}, {"_id": 1, "name": 1, "parent": 1}))
                screenids = [scr["_id"] for scr in screendetails]
                testcasedetails = list(dbsession.testcases.find({"screenid": {"$in": screenids}}, {
                                       "_id": 1, "name": 1, "parent": 1, "screenid": 1}))
                res = {'rows': {'screenList': screendetails,
                                'testCaseList': testcasedetails}}
            else:
    ...
    def checkScenarioNameExists(projectid, name):
        res = list(dbsession.testscenarios.find(
            {"projectid": ObjectId(projectid), "name": name}, {"_id": 1}))
        if len(res) > 0:
            return True
        else:
                ...
                for moduledata in requestdata['testsuiteDetails']:
                    if moduledata['testsuiteId'] is None:
                        if(checkModuleNameExists(moduledata["testsuiteName"], projectid)):
                            ...
                            currentmoduleid = saveTestSuite(
                                projectid, moduledata['testsuiteName'], versionnumber, createdthrough, userid, userroleid, type)
                    else:
                        oldModulename = getModuleName(
                            moduledata['testsuiteId'])
                        if oldModulename != moduledata["testsuiteName"]:
                            if(checkModuleNameExists(moduledata["testsuiteName"], projectid)):
                                ...
                                updateModuleName(
                                    moduledata["testsuiteName"], projectid, moduledata['testsuiteId'], userid, userroleid)
                        currentmoduleid = moduledata['testsuiteId']
                    for scenariodata in moduledata['testscenarioDetails']:
                        if scenariodata["state"] == "created":
                            if(checkScenarioIDexists(scenariodata["testscenarioName"], scenariodata["testscenarioid"])):
                                scenarioids.append(
                                    {"_id": ObjectId(scenariodata["testscenarioid"]), "screens": []})
                                updateparent(
                                    "scenarios", scenariodata["testscenarioid"], currentmoduleid, "add")
                            else:
                                error = "fail"
                                break
                        else:
    ...
    def checkScenarioIDexists(name, id):
        res = list(dbsession.testscenarios.find(
            {"_id": ObjectId(id), "name": name, "deleted": False}, {"_id": 1}))
        if len(res) == 1:
            return True
        else:
            return False
    ...
    def updateModuleName(modulename, projectid, moduleid, userid, userroleid):
        modifiedon = datetime.now()
        dbsession.mindmaps.update_one({"_id": ObjectId(moduleid)}, {"$set": {
                                      "name": modulename, "modifiedby": userid, "modifedon": modifiedon, "modifiedbyrole": userroleid}})
        return

    def updateScenarioName(scenarioname, projectid, scenarioid, userid, userroleid):
        modifiedon = datetime.now()
        dbsession.testscenarios.update_one({"_id": ObjectId(scenarioid)}, {"$set": {"name": scenarioname, "modifiedby": ObjectId(
            userid), "modifedon": modifiedon, "modifiedbyrole": ObjectId(userroleid)}})
        return

    def updateScreenName(screenname, projectid, screenid, userid, userroleid):
        modifiedon = datetime.now()
        dbsession.screens.update_one({"_id": ObjectId(screenid)}, {"$set": {"name": screenname, "modifiedby": ObjectId(
            userid), "modifedon": modifiedon, "modifiedbyrole": ObjectId(userroleid)}})
        return

    def updateTestcaseName(testcasename, projectid, testcaseid, userid, userroleid):
        modifiedon = datetime.now()
        dbsession.testcases.update_one({"_id": ObjectId(testcaseid)}, {"$set": {"name": testcasename, "modifiedby": ObjectId(
            userid), "modifedon": modifiedon, "modifiedbyrole": ObjectId(userroleid)}})
        return

    def getModuleName(moduleid):
        modulename = list(dbsession.mindmaps.find(
            {"_id": ObjectId(moduleid), "deleted": False}, {"name": 1}))
        if len(modulename) != 0:
    ...
    def getScenarioName(scenarioid):
        scenarioname = list(dbsession.testscenarios.find(
            {"_id": ObjectId(scenarioid), "deleted": False}, {"name": 1}))
    ...
    def getScreenName(screenid):
        screename = list(dbsession.screens.find(
            {"_id": ObjectId(screenid), "deleted": False}, {"name": 1}))
        if len(screename) != 0:
    ...
    def getTestcaseName(testcaseid):
        testcasename = list(dbsession.testcases.find(
            {"_id": ObjectId(testcaseid), "deleted": False}, {"name": 1}))
    ...
    def getTestcaseID(screenid, testcasename):
        testcaseid = list(dbsession.testcases.find({"screenid": ObjectId(
            screenid), "name": testcasename, "deleted": False}, {"_id": 1}))
        if len(testcaseid) != 0:
    ...
    def getScreenID(screenname, projectid):
        screenname = list(dbsession.screens.find({"projectid": ObjectId(
            projectid), "name": screenname, "deleted": False}, {"_id": 1}))
        if len(screenname) == 1:
                            ...
                            for scenariodata in scenarioList:
                                ...
                                for screendata in scenariodata['screens']:
                                    ...
                                    queryresult=dbsession.screens.update_many({'_id':ObjectId(currentscreenid),'versionnumber':versionnumber},
                                                {'$set':{'modifiedby':ObjectId(createdby),'modifiedbyrole':ObjectId(createdbyrole),'screenshot':screendata['screenshot'],'scrapedurl':screendata['scrapedurl'],"modifiedon" : datetime.now()}}).matched_count
                                    iddata2 = {"_id": ObjectId(
                                        currentscreenid), "testcases": []}
