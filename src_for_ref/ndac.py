...
currdir=os.getcwd()
config_path = currdir+'/ndac/src_for_ref/server_config.json'
...
def fetchCompleteTableData(tableName,cols):
    app.logger.debug("Inside fetchCompleteTableData tablename: "+str(tableName))
    ...
    if(data.current_rows[0]["count"]<=5000):
        ...
        for page in icesession.execute(executequery1):
            rep.append(page)
        app.logger.debug("Length of the rep: "+str(len(rep)))
        dumpData = rep
        n = 10000
...
def fetchDeltaTableData(tableName,cols,dumpTime):
    app.logger.debug("Inside fetchDeltaTableData tablename: "+str(tableName))
...
def main():
    ...
    try:
        ...
        if dashboard_conf.has_key('datadump'):
            ...
            if(dump_val.lower() == 'true'):
                ...
                app.logger.debug("Current Dashboard dumplag value: "+ str(dashboardDumpLag))
    ...
    try:
        ...
        if not mongoSession.server_info():
            raise ConnectionError("Unable to connect to PluginsDB")
    except Exception as e:
        app.logger.debug(e)
